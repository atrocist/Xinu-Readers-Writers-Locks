...
void writer2 (char *msg, int lck)
{
    ...
    sleep (3);
    testval = testval-5;
    if(SYSERR==releaseall (1, lck)){
	//	kprintf("writer 2 failed to release all locks\n");
	}
}
...
void test2 ()
{
    ...
    kill(pid2);
	kprintf("testval = %d\n",testval);
    assert (testval == 40,"Test 2 FAILED test val\n");
...
void writer4 (char *msg, int lck)
{
    ...
    sleep (5);
    if(SYSERR == releaseall (1, lck)){
		//kprintf("release all failed writer 4");
	}
...
void test4 ()
{
    ...
    resume(pid2);
    sleep(1);
    if(SYSERR == ldelete (lck)){
		kprintf("ldelete failed writer 4");
	}
